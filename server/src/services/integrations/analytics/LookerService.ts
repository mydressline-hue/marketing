 ...
 *
 * Manages Looker dashboards, data exports, connection status, and sync
 * operations against the `analytics_dashboards`, `analytics_exports`, and
 * `analytics_connections` tables (platform_type = 'looker'). All mutations
 * are audit-logged and relevant caches are invalidated.
 */
...
export class LookerService {
  ...
  static async exportData(userId: string, queryConfig: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    const result = await pool.query(
      `INSERT INTO analytics_exports
       ...
       RETURNING *`,
      [id, userId, PLATFORM, 'query', 0, 'completed', JSON.stringify(queryConfig)],
    );
    ...
    logger.info('Looker data export completed', { exportId: id, userId });
    return row;
  }
  ...
  static async createDashboard(userId: string, config: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    logger.info('Looker dashboard created', { dashboardId: id, userId });
    return dashboard;
  }
  ...
  static async updateDashboard(
    ...
    if (result.rows.length === 0) throw new NotFoundError(`Looker dashboard not found: ${dashboardId}`);
    ...
    logger.info('Looker dashboard updated', { dashboardId, userId });
    return dashboard;
  }
  ...
  static async deleteDashboard(userId: string, dashboardId: string): Promise<Record<string, unknown>> {
    ...
    logger.info('Looker dashboard deleted', { dashboardId, userId });
  ...
  static async refreshData(userId: string, dataSourceId: string): Promise<Record<string, unknown>> {
    ...
    logger.info('Looker data refresh triggered', { exportId: id, userId, dataSourceId });
    return row;
  }

  /**
   * Check the Looker connection status for a user. Cached.
   */
